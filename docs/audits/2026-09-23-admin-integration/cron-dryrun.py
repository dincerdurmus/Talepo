...
all_routes = "urgent-nudge match-backfill feature-expiry overdue-complaints category-provisioning"
cases = [("*/5 * * * *", "", "urgent-nudge"), ("*/15 * * * *", "", "match-backfill feature-expiry"), ("0 * * * *", "", "overdue-complaints"), ("0 3 * * *", "", "category-provisioning"), ("", "", all_routes), ("", "match-backfill", "match-backfill")]
with tempfile.TemporaryDirectory(prefix="talepo-cron-check-") as directory:
    ...
    for secret, code, exit_code, count in [("test-only", "200", 0, 5), ("test-only", "401", 1, 5), ("test-only", "503", 1, 5), ("", "200", 0, 0)]:
        log.write_text("")
...
print("10 passed; no network, deploy, checkout or migration.")
