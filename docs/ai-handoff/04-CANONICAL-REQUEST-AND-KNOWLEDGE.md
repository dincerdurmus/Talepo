...
| değer taşımayan cevap (`UNKNOWN` / `NOT_APPLICABLE` / `ANY`) | Bilinçli KULLANICI CEVABIDIR; cevaplanmamış alanla aynı değildir. Ayrım kaynağa bağlıdır: yalnız `EXPLICIT_TEXT` / `EXPLICIT_BROWSE` sayılır — varsayılan `UNKNOWN` (kapsam tabanında 988 alan, hepsi `INFERRED`) hiçbir yüzey üretmez. Tek karar noktası `answer-authority.ts` → `isDeliberateNonValueAnswer` [`CODE-VERIFIED` — `61fdcc1`, Karar J1] |
| cevap dispozisyon yüzeyi (`fieldResponses`) | `UNKNOWN` / `NOT_APPLICABLE` için additive projection yüzeyi; `kind` + `authority: USER_EXPLICIT` taşır. `ANY` buraya GİRMEZ — kendi `constraints[key].mode = "ANY"` kanalındadır. İstemcinin gönderdiği kopya güven sınırında ATILIR ve yalnız süzülmüş cevap kanalından (`fields[] + mode`) yeniden türetilir; metin türetimi bu yüzeyde kullanılamaz [`CODE-VERIFIED` — `61fdcc1`, Karar J2] |
| değer taşımayan cevabın kalıcılığı | `RequestFieldValue.jsonValue = { mode }`, `textValue = null`. **Migration yoktur** — mevcut kolon kullanılır. Çelişkide structured mod kazanır; legacy `textValue="Fark etmez"` `VALUE` okunur ve backfill yapılmaz. Dedicated `budget` / `city` / `deadline` kolonlarına sahte `0` / etiket / tarih yazılmaz [`CODE-VERIFIED` — `61fdcc1`, Karar J3–J4] |
| cevap evreni (`publishAnswerKeyUniverse`) | Kategori registry alanları + o kategorinin soru profili anahtarları (`listProfileKeysForCategory`) + üretilmeyen ortak alanlar; üstünde `isProjectionAuthorityKeyAllowed`. Soru ZAMANLAMA süzgecinden (`whenProductTypes` / `whenNeedTypes`) bağımsızdır ama MEVCUT kategoriyle sınırlıdır. Kabul kategorisi sunucunun kalıcılaştırdığı `category.slug`'dan okunur, `discoveryProjection.categoryId`'den değil; tanınmayan/bozuk değer evreni yalnız daraltır. İstemci ve sunucu AYNI fonksiyonu okur [`CODE-VERIFIED` — `61fdcc1`, Karar J7–J8] |
| generated `title` | Cevap alanı DEĞİL, `composeRequestTitle` ile üretilen etiket. `fields[]`, `attributes`, `constraints`, `fieldAuthority`, `fieldResponses` ve `fieldConfirmations` yüzeylerinin hiçbirinde görünmez; istemci zorlarsa fail-closed düşer. `Request.title` NOT NULL korunur, migration yoktur. Kanonik işaret `COMMON_FIELD_DEFAULTS.title.generated = true` → `isGeneratedCommonField` [`CODE-VERIFIED` — `61fdcc1`, Karar J6] |
| answer freshness (`FRESH` / `INHERITED`) | Otoriteden AYRI eksen. `Authority` "bunu kim söyledi?" (eskimez), `AnswerFreshness` "bugün hâlâ onaylı mı?" sorusunu cevaplar; bir alan aynı anda `USER_EXPLICIT` ve `INHERITED` olabilir. Süre eşiği yoktur: karar `status` + cevaba bağlı deterministik `answerSignature` (FNV-1a, `v1:xxxxxxxx`, ham PII taşımaz) ile verilir. `localStorage` / URL tazelik kanıtı değildir. Miras ortak cevap (`budget` / `city` / `delivery` / `quantity`) kullanıcı "aynı kalsın / değiştir" demeden güncel sayılmaz ve kaydetme ilerlemez [`CODE-VERIFIED` — `61fdcc1`, Karar K] |
| `confirmedFieldKeys` (küme semantiği) | Onay bir SAYIM değil ÜYELİK iddiasıdır: aynı anahtar en fazla bir kez bulunur. Tekilleştirme kanonik sınırda (`buildUnderstandingSnapshot`) yapılır ve kırpma (80) tekilleştirmeden SONRA uygulanır; çağrı yerlerinde tek tek temizlenmez [`CODE-VERIFIED` — `61fdcc1`, Karar J9] |
