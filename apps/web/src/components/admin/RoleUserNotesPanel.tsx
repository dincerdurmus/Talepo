...
type Note = { id: string; body: string; createdAt: string; author: { name: string | null; email: string | null; platformRole: string } };
...
export function RoleUserNotesPanel({ user, onClose }: { user: { id: string; label: string; role: string }; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/role-notes?userId=${encodeURIComponent(user.id)}`, { signal: controller.signal })
      .then(async (response) => { const data = await response.json() as { notes?: Note[]; events?: Event[]; message?: string }; if (!response.ok) throw new Error(data.message ?? "İç notlar alınamadı."); setNotes(data.notes ?? []); setEvents(data.events ?? []); })
  ...
  async function save() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/role-notes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id, body }) });
  ...
  return <div className="fixed inset-0 z-[220] flex justify-end bg-black/60 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside role="dialog" aria-modal="true" aria-labelledby="role-notes-title" className="h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-[#091915] p-6 shadow-2xl">
      ...
      <div className="mt-6 rounded-2xl border border-sky-300/15 bg-sky-300/[.06] p-4"><label htmlFor="role-note" className="text-sm font-semibold text-sky-100">Yeni iç not</label><p className="mt-1 text-xs leading-5 text-white/40">Bu not kullanıcıya ve diğer rollere görünmez.</p><textarea id="role-note" value={body} onChange={(event) => setBody(event.target.value)} rows={4} maxLength={2000} placeholder="Örn. Bu Support kullanıcısının kapattığı vakalar haftalık olarak takip edilecek." className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-black/20 p-3 text-sm outline-none placeholder:text-white/25 focus:border-sky-300/40"/><div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs text-white/35">{body.length}/2000</span><button type="button" disabled={saving || body.trim().length < 3} onClick={() => void save()} className="inline-flex items-center gap-2 rounded-xl bg-sky-200 px-3.5 py-2 text-sm font-semibold text-[#08211d] disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <StickyNote className="h-4 w-4" />}Notu kaydet</button></div></div>
