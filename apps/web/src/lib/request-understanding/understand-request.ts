...
export function understandRequest(
  ...
  /**
   * Karar sınırı (2026-09-20): kategori, kanonik varlık ve ürün kimliği
  ...
  let category = decisions.decideRequestCategory({
   ...
   *
   * Kategori kararı (category-gate) metindeki EN UZUN tam kanonik yaprak eşleşmesiyle kesin
   ...
   */
  const domainEntityHit = decisions.resolveCanonicalEntity({
    text: normalizedInput,
  });
  ...
  if (usageContext && !categoryFromCanonicalClaim() && !relationDomain?.categoryId) {
    ...
    const fromTarget = decisions.decideRequestCategory({
      text: usageContext.target,
      intent: intentResolved.intent,
    });
    if (
      fromTarget.value &&
