...
export async function writeAdminAudit(tx: Prisma.TransactionClient, input: AuditInput) {
  ...
  return tx.adminAuditLog.create({ data: { actorId: input.actorId, targetUserId: input.targetUserId ?? null, action: input.action as never, reason: input.reason.trim(), before: input.before, after: input.after, metadata: input.metadata, ipHash, correlationId: input.request?.headers.get("x-correlation-id") ?? null } });
}
