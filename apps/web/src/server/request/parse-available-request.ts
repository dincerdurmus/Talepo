...
export async function parseAvailableRequestInput(body: unknown, options?: Parameters<typeof parseCreateRequestInput>[1]) {
  ...
  return parseCreateRequestInput(body, { ...options, availableCategories: rows.map(publicCategoryDefinition) });
}
