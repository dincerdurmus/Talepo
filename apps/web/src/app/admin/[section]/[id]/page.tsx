...
import { CompanyOperationsPanel } from "@/components/admin/CompanyOperationsPanel";
import { CompanySeatAssignment } from "@/components/admin/CompanySeatAssignment";
...
export default async function AdminDetailPage({ params }: { params: Promise<{ section: string; id: string }> }) {
  ...
  const company = section === "companies" ? { id: String(record.id), name: String(record.name ?? "Firma"), status: String(record.status ?? "ACTIVE"), planTier: String(record.planTier ?? "STANDARD") } : null;
  const fields = Object.entries(record).filter(([key]) => key !== "members" && key !== "_count");
  return <AdminShell name={admin.name} role={admin.platformRole} title="Kayıt detayı"><Link href={`/admin/${section}`} className="text-sm text-primary hover:text-primary">← Listeye dön</Link><div className="mt-8 rounded-xl border border-border bg-card p-6"><p className="text-xs uppercase tracking-[.2em] text-amber-700">Admin detay</p><h1 className="mt-2 text-3xl font-semibold">{String(record.title ?? record.name ?? record.id)}</h1><dl className="mt-8 grid gap-4 sm:grid-cols-2">{fields.map(([key, value]) => <div key={key} className="rounded-2xl border border-border bg-muted p-4"><dt className="text-xs uppercase tracking-[.12em] text-muted-foreground">{fieldLabels[key] ?? key}</dt><dd className="mt-2 break-words text-sm text-foreground">{key === "seats" && section === "companies" ? <CompanyMembersDrawer members={members}>{String(value)}</CompanyMembersDrawer> : key === "owner" && section === "companies" && typeof value === "object" && value !== null && "id" in value ? <UserDetailDrawer user={value as { id: string; name: string | null; email: string | null; membershipNumber: string }}>{String("name" in value ? value.name : "Firma sahibi")}</UserDetailDrawer> : formatValue(key, value)}</dd></div>)}</dl></div>{company && hasAdminPermission(admin.platformRole, "companies.manage") ? <CompanySeatAssignment companyId={company.id} /> : null}{company && hasAdminPermission(admin.platformRole, "billing.manage") ? <CompanyOperationsPanel company={company} /> : null}</AdminShell>;
}

const fieldLabels: Record<string, string> = { id: "Kimlik", name: "Firma adı", legalName: "Yasal ad", status: "Durum", planTier: "Plan", email: "E-posta", phone: "Telefon", taxNumber: "Vergi numarası", taxOffice: "Vergi dairesi", country: "Ülke", city: "Şehir", district: "İlçe", neighborhood: "Mahalle", address: "Adres", createdAt: "Oluşturulma tarihi", seats: "Koltuk sayısı", requests: "Talep sayısı", offers: "Teklif sayısı", title: "Başlık", description: "Açıklama", offerCount: "Teklif sayısı", publishedAt: "Yayınlanma tarihi", owner: "Firma sahibi", category: "Kategori" };
