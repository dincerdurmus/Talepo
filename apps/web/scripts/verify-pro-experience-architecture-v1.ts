...
assert.equal(PRO_VALUE_PILLARS[0].title, "Keşfet");
assert.equal(PRO_VALUE_PILLARS[1].title, "Karar ver");
assert.equal(PRO_VALUE_PILLARS[2].title, "Ölç / geliştir");
assert.equal(PRO_VALUE_PILLARS[3].title, "Takip et");
assert.equal(FEATURE_META.ai_offer_assistant.label, "Teklif taslağı");
assert.ok(PRO_VALUE_PILLARS[0].features.includes("talepo_radar"));
assert.ok(PRO_VALUE_PILLARS[1].features.includes("professional_analytics"));
